...
router.get('/summary', authRequired, async (req, res) => {
  ...
  const now = new Date();
  const activeParking = await ParkingBooking.count({ where: { status: 'booked', start_time: { [Op.gte]: now } } });
  const meetingRooms = await MeetingRoom.count();
  const todayMeetings = await MeetingBooking.count({
    where: {
      start_time: { [Op.gte]: new Date(new Date().setHours(0,0,0,0)) },
      end_time: { [Op.lte]: new Date(new Date().setHours(23,59,59,999)) }
    }
  });
  res.json({
